...
import { ArrowDown, ArrowUp } from "lucide-react"; // Example icons
...
const getSortIcon = (key: "date" | "price", query: ISalesQuery) => {
  if (query.sortBy !== key) return null;
  return query.sortOrder === "asc" ? <ArrowUp size={14} /> : <ArrowDown size={14} />;
};

export const SalesTable = ({ sales, currentQuery, onSort }: SalesTableProps) => {
  return (
      ...
      <tbody className="bg-white divide-y divide-gray-200">
        {sales.map((sale) => (
          <tr key={sale._id} className="hover:bg-gray-50">
            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 truncate max-w-[100px]">{sale._id}</td>
            ...
            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{sale.customerPhone}</td>
          </tr>
        ))}
      </tbody>
    </table>
